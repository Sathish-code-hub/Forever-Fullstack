...
const Login = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-700'>
        ...
        {currentState === 'Login' ? '' : 
        <input onChange={(e) => setName(e.target.value)} value={name} type='text' placeholder='Name' className='border border-gray-800 w-full px-3 py-2' required />}
        ...
        <button className='bg-black text-white px-6 py-2'>{currentState === 'Login' ? 'Login' : 'Sign In'}</button>
